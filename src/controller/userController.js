...
const userRegister = async (req, res) => {
    try {
        ...
        if (typeof jsonAddress != 'object') return res.status(400).send({ status: false, message: "Address must be in object" });
        ...
        if (jsonAddress) {
            if (jsonAddress.shipping) {
                ...
                if (!(/^\d{6}$/).test(jsonAddress.shipping.pincode)) return res.status(400).send({ status: false, message: "Pincode should in six digit Number" })
            ...
            if (jsonAddress.billing) {
                ...
                if (!(/^\d{6}$/).test(jsonAddress.billing.pincode)) return res.status(400).send({ status: false, message: "Pincode should in six digit Number" })
...
const getUser = async (req, res) => {
    try {
        ...
        if (req.userId !== userId) return res.status(403).send({ status: false, message: "UnAuthorised access" })
...
const updateUser = async (req, res) => {
    ...
    try {
        ...
        if (data?.address) {
            ...
            if (data?.address?.shipping) {
                if (data?.address?.shipping?.street) {
                    if (!isValid(data.address.shipping.street)) return res.status(400).send({ status: false, Message: "Please provide your street name in shipping address" })
                }
                if (data?.address?.shipping?.city) {
                    if (!isValid(data.address.shipping.city)) return res.status(400).send({ status: false, Message: "Please provide your city name in shipping address" })
                }
                if (data?.address?.shipping?.pincode) {
                    if (!isValid(data.address.shipping.pincode)) return res.status(400).send({ status: false, Message: "Please provide your pin code in shipping address" })
                    if (!(/^\d{6}$/).test(jsonAddress.billing.pincode)) return res.status(400).send({ status: false, message: "Pincode should in six digit Number" })
            ...
            if (data?.address?.billing) {
                if (data?.address?.billing?.street) {
                    if (!isValid(data.address.billing.street)) return res.status(400).send({ status: false, Message: "Please provide your street name in billing address" })
                }
                if (data?.address?.billing?.city) {
                    if (!isValid(data.address.billing.city)) return res.status(400).send({ status: false, Message: "Please provide your city name in billing address" })
                }
                if (data?.address?.billing?.pincode) {
                    if (!isValid(data.address.billing.pincode)) return res.status(400).send({ status: false, Message: "Please provide your pin code in billing address" })
                    if (!(/^\d{6}$/).test(jsonAddress.billing.pincode)) return res.status(400).send({ status: false, message: "Pincode should in six digit Number" })
        ...
        let dataTobeUpdate = {
            fname: data.fname, lname: data.lname, email: data.email, phone: data.phone, password: data.password, profileImage: data.profileImage,
            address: {
                shipping: {
